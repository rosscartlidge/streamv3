...
type CodeFragment struct {
	...
	Command string   `json:"command"` // The streamv3 command that generated this fragment (e.g., "streamv3 read-csv")
}
...
func AssembleCodeFragments(input io.Reader) (string, error) {
	...
	// Add command pipeline as comments
	var commands []string
	for _, frag := range fragments {
		if frag.Command != "" {
	...
	if len(commands) > 0 {
		...
		// Remove trailing " |" from last command
		if len(commands) > 1 {
			lastLineStart := findString(code, "//   "+commands[len(commands)-1])
			if lastLineStart != -1 {
				lineEnd := lastLineStart + len("//   "+commands[len(commands)-1]) + 2 // +2 for " |"
				code = code[:lineEnd-2] + code[lineEnd:]
			}
		}
		code += "\n"
	}
