...
// ============================================================================
// SQL-STYLE FLUENT EXTENSIONS
// ============================================================================

// SQL-style convenience methods for StreamBuilder

// GroupRecordsBy groups records by a key extraction function
func GroupRecordsBy[K comparable](sb *StreamBuilder[Record], keyFn func(Record) K) *StreamBuilder[GroupedRecord] {
	return TransformTo(sb, GroupBy(keyFn))
}

// GroupRecordsByFields groups records by specified field values
func GroupRecordsByFields(sb *StreamBuilder[Record], fields ...string) *StreamBuilder[GroupedRecord] {
	return TransformTo(sb, GroupByFields(fields...))
}

// AggregateGroups applies aggregation functions to grouped records
func AggregateGroups(sb *StreamBuilder[GroupedRecord], aggregations map[string]AggregateFunc) *StreamBuilder[Record] {
	return TransformTo(sb, Aggregate(aggregations))
}

// InnerJoinRecords performs an inner join between two record streams
func InnerJoinRecords(left *StreamBuilder[Record], right *StreamBuilder[Record], predicate JoinPredicate) *StreamBuilder[Record] {
	return left.Apply(InnerJoin(right.Iter(), predicate))
}

// LeftJoinRecords performs a left join between two record streams
func LeftJoinRecords(left *StreamBuilder[Record], right *StreamBuilder[Record], predicate JoinPredicate) *StreamBuilder[Record] {
	return left.Apply(LeftJoin(right.Iter(), predicate))
}

// RightJoinRecords performs a right join between two record streams
func RightJoinRecords(left *StreamBuilder[Record], right *StreamBuilder[Record], predicate JoinPredicate) *StreamBuilder[Record] {
	return left.Apply(RightJoin(right.Iter(), predicate))
}

// FullJoinRecords performs a full outer join between two record streams
func FullJoinRecords(left *StreamBuilder[Record], right *StreamBuilder[Record], predicate JoinPredicate) *StreamBuilder[Record] {
	return left.Apply(FullJoin(right.Iter(), predicate))
}

// Note: Join operations for Records are implemented as standalone functions
// Use InnerJoinRecords(left, right, predicate), etc.

// Note: GroupBy with type parameters is implemented as a standalone function
// Use GroupRecordsBy[K](sb, keyFn) instead

// Note: Record operations are implemented as standalone functions
// Use GroupRecordsByFields(sb, fields...) and AggregateGroups(sb, aggregations)

// ============================================================================
// SELECT OPERATIONS
// ============================================================================

// Select projects only specified fields from records (SQL SELECT equivalent)
func Select(fields ...string) FilterSameType[Record] {
	return func(input iter.Seq[Record]) iter.Seq[Record] {
		return func(yield func(Record) bool) {
			for record := range input {
				selected := make(Record)
				for _, field := range fields {
					if val, exists := record[field]; exists {
						selected[field] = val
					}
...
func AddField(name string, valueFn func(Record) any) FilterSameType[Record] {
	return func(input iter.Seq[Record]) iter.Seq[Record] {
		return func(yield func(Record) bool) {
			for record := range input {
				// Create a copy with the new field
				result := make(Record)
				for k, v := range record {
					result[k] = v
				}
				result[name] = valueFn(record)
				if !yield(result) {
					return
				}
			}
...
// SelectRecordFields projects only specified fields from records
func SelectRecordFields(sb *StreamBuilder[Record], fields ...string) *StreamBuilder[Record] {
	return sb.Apply(Select(fields...))
}

// AddRecordField adds a computed field to each record
func AddRecordField(sb *StreamBuilder[Record], name string, valueFn func(Record) any) *StreamBuilder[Record] {
	return sb.Apply(AddField(name, valueFn))
}